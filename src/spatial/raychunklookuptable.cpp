...
RayChunkLookupTable::RayChunkLookupTable(game::GameContext &context)
    : context(context)
{}

/*
static const char *filename = "RayChunkLookupTable_cache.bin";
...
bool RayChunkLookupTable::loadSequences(const char *filename) {
...
void RayChunkLookupTable::writeSequences(const char *filename) {
...
void RayChunkLookupTable::generateSequences() {
    context.get<spdlog::logger>().debug("Sizeof(RayChunkLookupTable::sequences) = {}", sizeof(sequences));
...
const RayChunkLookupTable::SearchSequence &RayChunkLookupTable::lookup(glm::vec3 origin, glm::vec3 dir, float parallelDist, float perpDistSq) {
...
RayChunkLookupTable::SearchSequence &RayChunkLookupTable::getSequence(glm::vec3 origin, glm::vec3 dir, float parallelDist, float perpDistSq) {
    unsigned int osx;
    unsigned int osy;
    unsigned int osz;
    if (originSplits == 1) {
...
void RayChunkLookupTable::generateSingleSequence(SearchSequence &searchSequence) {
...
RayChunkLookupTable::Query RayChunkLookupTable::searchSequenceToLookupQuery(const SearchSequence &searchSequence, const double offs[7]) {
...
void RayChunkLookupTable::setMinSurfaceDistance(SearchSequence::Cell &cell, const SearchSequence &searchSequence) {
    NloptUserData userData;
    userData.RayChunkLookupTable = this;
...
double RayChunkLookupTable::nloptFunc(unsigned int n, const double *x, double *grad, void *userData) {
...
double RayChunkLookupTable::getSurfaceDistForParams(const double *x, NloptUserData *userData) {
    assert(nloptArgCount == 10);
    Query query = userData->RayChunkLookupTable->searchSequenceToLookupQuery(*userData->searchSequence, x);
    return userData->RayChunkLookupTable->getSurfaceDistanceAlongRay<double>(query.origin, query.dir, query.parallelDist, query.perpDistSq, glm::dvec3(x[7], x[8], x[9]));
}
