...
void main(void) {
    ...
    uvec4 t0 = max(values & 0xFFu, values & 0xFF00u);
    uvec4 t1 = max(values & 0xFF0000u, values & 0xFF000000u);
    uvec4 t2 = max(t0, t1);
    uint t3 = max(max(t2.x, t2.y), max(t2.z, t2.w));
    // Make values equal to 255 become zero, and all other values be clipped
    float y = (float(t3) - 255.0) * 2.0;
