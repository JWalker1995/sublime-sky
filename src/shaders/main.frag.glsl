...
// Copied from https://github.com/wwwtyro/glsl-atmosphere
...
void main(void) {
    // Using the result of this is extremely slow for some reason
    //atomicAdd(faceCounts[gl_PrimitiveID], 1);
    //uint faceCount = faceCounts[gl_PrimitiveID];

    //atomicCounterIncrement(num_fragments);

    if (false /* mesh id == skygrid mesh id */) {
        // Render sky
        ...
        /*
        vec3 uSunPos = vec3(0.0, 0.1, -1.0);
        vec3 vPosition = orig_position.xyz;
